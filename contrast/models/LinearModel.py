...
class LinearClassifierResNet(nn.Module):
    def __init__(self, layer=6, n_label=1000, expansion=4, pool_type='avg', width=1):
        super(LinearClassifierResNet, self).__init__()
        base_width = 64 * expansion
        if layer == 1:
            pool_size = 8
            ...
            pool_size = 6
            n_channels = base_width * width
            pool = pool_type
        elif layer == 3:
            pool_size = 4
            n_channels = base_width * 2 * width
            pool = pool_type
        elif layer == 4:
            pool_size = 3
            n_channels = base_width * 4 * width
            pool = pool_type
        elif layer == 5:
            pool_size = 7
            n_channels = base_width * 8 * width
            pool = pool_type
        elif layer == 6:
            pool_size = 1
            n_channels = base_width * 8 * width
