...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, low_dim=128, in_channel=3, width=1, projection='linear'):
        self.inplanes = 64
        ...
        if projection == 'linear':
            ...
            # )
            self.fc = nn.Sequential(
                nn.Conv2d(self.base * 8 * block.expansion, self.base * 8 * block.expansion, 1, bias=False),
                nn.BatchNorm2d(self.base * 8 * block.expansion),
                nn.ReLU(inplace=True),
                nn.Conv2d(self.base * 8 * block.expansion, low_dim, 1, bias=False),
